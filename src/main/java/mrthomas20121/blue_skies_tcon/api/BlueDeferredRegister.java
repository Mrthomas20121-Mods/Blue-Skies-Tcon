...
/**
 * Copy of Forge DeferedRegister.
 *
 * @param <T> The base registry type, must be a concrete base class, do not use subclasses or wild cards.
 */
public class BlueDeferredRegister<T extends IForgeRegistryEntry<T>>
{
    /**
     * Use for vanilla/forge registries. See example above.
     */
    public static <B extends IForgeRegistryEntry<B>> BlueDeferredRegister<B> create(IForgeRegistry<B> reg, String modid)
    {
        return new BlueDeferredRegister<B>(reg, modid);
    }

    /**
     * Use for custom registries that are made during the NewRegistry event.
     */
    public static <B extends IForgeRegistryEntry<B>> BlueDeferredRegister<B> create(Class<B> base, String modid)
    {
        return new BlueDeferredRegister<B>(base, modid);
    }
    ...
    protected BlueDeferredRegister(Class<T> base, String modid)
    {
        this.superType = base;
        this.modid = modid;
    }

    protected BlueDeferredRegister(IForgeRegistry<T> reg, String modid)
    {
    ...
    {
        bus.register(new BlueDeferredRegister.EventDispatcher(this));
        if (this.type == null && this.registryFactory != null) {
            bus.addListener(this::createRegistry);
        }
    }
    public static class EventDispatcher {
        private final BlueDeferredRegister<?> register;

        public EventDispatcher(final BlueDeferredRegister<?> register) {
